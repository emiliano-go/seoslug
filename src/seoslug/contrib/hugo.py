"""Hugo integration for seoslug.

Generates SEO metadata for Hugo content files at build time. Handles
both TOML (``+++``) and YAML (``---``) frontmatter formats.
...
@dataclass
class HugoConfig:
    """Configuration for :class:`HugoBuilder`."""
    ...
    trailing_slash: str = "never"
    dry_run: bool = False


class HugoBuilder:
    """Build SEO metadata for Hugo content files.

    Walks the Hugo content directory, parses frontmatter, builds SEO
    payloads with seoslug, and injects the rendered HTML into each
    ...
    Args:
        ...
        trailing_slash: URL trailing slash policy (default ``never``
            for Hugo).
        dry_run: When True, log changes but do not write files.

    Note:
        ``HugoBuilder`` is a library class. Use it by importing and
    ...
    def _parse_frontmatter(self, text: str) -> tuple[dict[str, Any], str]:
        """Parse Hugo frontmatter (TOML ``+++`` or YAML ``---``).
    ...
    def _write_frontmatter(self, filepath: Path, meta: dict, body: str) -> None:
        """Write YAML frontmatter back to the file.
...
def build_hugo(
    ...
    **kwargs: Any,
    ...
    """Convenience function for one-shot Hugo SEO generation.

    Shortcut for creating a :class:`HugoBuilder` and calling
    :meth:`HugoBuilder.build`.
