...
Starlette shares the same async pattern as FastAPI. Use `build_seo_payload_async` directly in your endpoint. The main difference is that Starlette does not provide dependency injection, so you create the config manually:
...
For JSON endpoints, use `build_seo_payload_async` and call `.to_dict()` on the result, or use the synchronous `build_seo_payload_dict` (which runs in the current thread).
